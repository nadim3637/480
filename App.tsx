...
const App: React.FC = () => {
  ...
  useEffect(() => {
      ...
      const unsubscribe = subscribeToSettings(async (newSettings) => {
          if (newSettings) {
              setState(prev => {
              ...
              if (newSettings.forceRefreshTimestamp) {
                  const lastRefresh = await storage.getItem('nst_last_refresh_ts');
  ...
  useEffect(() => {
      const initApp = async () => {
          ...
          const storedSettings = await storage.getItem('nst_system_settings');
          if (storedSettings) {
              try {
          ...
          const darkModeVal = await storage.getItem('nst_dark_mode');
          ...
          const loggedInUserStr = await storage.getItem('nst_current_user');
          const today = new Date().toDateString();

          if (loggedInUserStr) {
              // 2. Daily Tracker (Once per day)
              const lastTracker = await storage.getItem('nst_last_daily_tracker_date');
              if (lastTracker !== today) {
                  queue.push('TRACKER');
              }

              // 3. Daily Challenge (Once per day)
              const lastChallenge = await storage.getItem('nst_last_daily_challenge_date');
          ...
          console.log("Restoring user from storage:", loggedInUserStr);
          if (loggedInUserStr) {
            try {
  ...
  useEffect(() => {
    ...
    const loadTimer = async () => {
        const storedDate = await storage.getItem('nst_timer_date');
        const storedSeconds = parseInt(await storage.getItem('nst_daily_study_seconds') || '0');
  ...
  const logActivity = async (action: string, details: string, overrideUser?: User) => {
      ...
      const storedLogs = await storage.getItem('nst_activity_log');
  ...
  const handleLogin = (user: User) => {
    ...
    // AUTO-VOICE LOGIN
    const welcomeText = state.settings.welcomeMessage ? state.settings.welcomeMessage.replace('{name}', user.name) : `Welcome ${user.name}`;
  ...
  const handleContentGeneration = async (type: ContentType, count?: number, forcePay: boolean = false) => {
    ...
    if (type === 'NOTES_HTML_FREE' || type === 'NOTES_HTML_PREMIUM' || type === 'NOTES_IMAGE_AI') {
        ...
        if (!contentData) {
            ...
            if (stored) contentData = stored; // storage.getItem already parses JSON if saved as object, but here we expect object
        }
            ...
            if (!state.originalAdmin) {
                ...
                if (storedUsers) {
                    ...
                    if (idx !== -1) { 
                        ...
                        storage.setItem('nst_users', JSON.stringify(allUsers)); 
                    }
                }
                // Sync to Live
                saveUserToLive(updatedUser);
  ...
  const handleWeeklyTestComplete = async (score: number, total: number, answers: Record<number, number>) => {
    ...
    const attempt = {
        ...
        startedAt: (await storage.getItem(`weekly_test_start_${activeWeeklyTest.id}`)) || new Date().toISOString(),
    ...
    const storedAttempts = await storage.getItem(key);
    ...
    const startTimeStr = await storage.getItem(`weekly_test_start_${activeWeeklyTest.id}`);
  ...
  if (!isAppReady) {
      return (
          <div className="min-h-screen bg-white flex flex-col items-center justify-center p-8">
              ...
              <h2 className="text-xl font-bold text-slate-800 animate-pulse">Starting App...</h2>
          </div>
      );
  }
