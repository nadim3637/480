...
export const LessonView: React.FC<Props> = ({ 
  ...
  const handleBilingualToggle = async () => {
      ...
      // Check if Hindi Content Exists
      if (content.schoolPremiumNotesHtml_HI || content.manualMcqData_HI) {
          setLanguage('Hindi');
          return;
      }

      // GENERATE MISSING CONTENT
      if (confirm("Hindi content not available on Cloud. Generate with AI? (Takes ~30s)")) {
          setIsTranslating(true);
          try {
              // 1. Generate Hindi Version
              const newContent = await fetchLessonContent(
                  ...
                  "",
                  true, // Allow AI
                  'SCHOOL', // Force School Mode for notes
                  true // Force Regenerate
              );

              // 2. Merge & Save
              if (newContent.content || newContent.mcqData) {
                  const updatedContent = { 
                      ...content, 
                      schoolPremiumNotesHtml_HI: newContent.content,
                      manualMcqData_HI: newContent.mcqData
                  };
                  
                  // Update Storage & Firebase
                  const streamKey = (classLevel === '11' || classLevel === '12') && user?.stream ? `-${user.stream}` : '';
                  const key = `nst_content_${user?.board || 'CBSE'}_${classLevel}${streamKey}_${subject.name}_${chapter.id}`;
                  
                  await storage.setItem(key, updatedContent);
                  await saveChapterData(key, updatedContent);
                  
                  // Update UI (Dirty fix: Reload or Update Parent)
                  // Ideally call onUpdateUser or similar, but here we can just setLanguage and hope parent re-renders or local content is used?
                  // LessonView uses 'content' prop. We can't mutate it.
                  // We need to force a re-render. 
                  // But wait, 'content' prop is immutable. 
                  // We need to notify parent (App.tsx).
                  // App.tsx doesn't have a specific callback for content update.
                  // However, App.tsx re-reads storage on navigation? No.
                  
                  // Hack: Mutate local object if possible or alert user to reload?
                  // Better: Add onContentUpdate prop?
                  // App.tsx has `handleContentGeneration` which updates state.
                  
                  // Since we can't easily update parent state from here without new prop, 
                  // we'll update storage (done) and force reload or just show alert.
                  // Or assume user will re-open.
                  
                  alert("✅ Hindi Content Generated & Saved! Re-opening lesson...");
                  window.location.reload(); // Simplest fix for now to refresh 'content' prop from storage
              }
          } catch(e) {
              console.error(e);
                  ...
                  <header className="bg-white/95 backdrop-blur-md text-slate-800 p-4 absolute top-0 left-0 right-0 z-10 flex items-center justify-between border-b border-slate-100 shadow-sm">
                      ...
                      <div className="flex items-center gap-1">
                          <button 
                              ...
                              className={`px-3 py-1.5 rounded-lg text-xs font-bold border mr-1 flex items-center gap-1 transition-all ${isTranslating ? 'bg-yellow-100 text-yellow-700 animate-pulse' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 border-slate-200'}`}
                          >
                              {isTranslating ? <div className="w-3 h-3 border-2 border-yellow-600 border-t-transparent rounded-full animate-spin"/> : <Globe size={14} />} 
                              {isTranslating ? 'AI Translating...' : (language === 'English' ? 'Hindi (हिंदी)' : 'English')}
                          </button>
               ...
               <div className="flex items-center justify-between p-4 bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm">
                   <div className="flex gap-2">
                       ...
                       <button 
                           onClick={handleBilingualToggle}
                           disabled={isTranslating}
                           className={`flex items-center gap-2 font-bold text-xs bg-slate-100 border border-slate-200 px-3 py-2 rounded-lg hover:bg-slate-200 transition-colors ${isTranslating ? 'text-yellow-600' : 'text-slate-600'}`}
                       >
                           {isTranslating ? <div className="w-3 h-3 border-2 border-yellow-600 border-t-transparent rounded-full animate-spin"/> : <Globe size={14} />} 
                           {isTranslating ? 'AI Translating...' : (language === 'English' ? 'English' : 'हिंदी')}
                       </button>
                       {!showResults && (
